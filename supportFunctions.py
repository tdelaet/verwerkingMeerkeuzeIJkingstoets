...
def calculateScoreAllPermutations(sheet_loc,blankAnswer_loc,matrixAnswers_loc,correctAnswers_loc,permutations_loc,alternatives_loc,numParticipants_loc,columnSeries_loc,content_colNrs_loc,scoreWrongAnswer_loc,scoreBlankAnswer_loc,scoreNeutralizedAnswer_loc,neutralized):
    ...
    neutralizedAnswersAllPermutations_loc= numpy.zeros((numSeries_loc,numParticipants_loc,numQuestions_loc))
    #Calculate score for all permutations
                 
    for question_loc in range(1,numQuestions_loc+1): #itereert over de vragen 
        #print("----------------------")
        ...
        for permutation in range(1,numSeries_loc+1):
            ...
            #print("questionLoc " + str(numQuestionPermutations_loc))
            #Check if neutralized
            if numQuestionPermutations_loc in neutralized:
                scoreQuestionsAllPermutations_loc[permutation-1,:,numQuestionPermutations_loc-1]+=scoreNeutralizedAnswer_loc
                neutralizedAnswersAllPermutations_loc[permutation-1,:,numQuestionPermutations_loc-1] =1.0
                #print(str(numQuestionPermutations_loc) + " in series " + str(permutation) + " is neutralized")
            else:    
                correctAnswer = correctAnswers_loc[numQuestionPermutations_loc-1]
                #print("correctAnswer " + str(correctAnswer))
    ...
    return scoreQuestionsAllPermutations_loc,correctAnswersAllPermutations_loc,wrongAnswersAllPermutations_loc,blankAnswersAllPermutations_loc,neutralizedAnswersAllPermutations_loc
...
def getNumberAlternatives(sheet_loc,content_loc,permutations_loc,columnSeries_loc,scoreQuestionsIndicatedSeries_loc,alternatives_loc,blankAnswer_loc,content_colNrs_loc):
    ...
    for question_loc in range(1,numQuestions_loc+1):
        ...
        if numAlternatives_loc == 5:        
            ...
            columnQuestion_loc = ['E' if x=='5'  else x for x in columnQuestion_loc]
            columnQuestion_loc = [blankAnswer_loc if x=='6'  else x for x in columnQuestion_loc]
        else:
...
def getScoreQuestionsIndicatedSeries(scoreQuestionsAllPermutations_loc,correctAnswersAllPermutations_loc,wrongAnswersAllPermutations_loc,blankAnswersAllPermutations_loc,neutralizedAnswersAllPermutations_loc,columnSeries_loc):
    ...
    neutralizedAnswersIndicatedSeries_loc= numpy.zeros((numParticipants_loc,numQuestions_loc))
    ...
    numberNeutralizedAnswers_loc = numpy.zeros((numParticipants_loc))
    ...
    for participant in range(numParticipants_loc):
        ...
        neutralizedAnswersIndicatedSeries_loc[participant,:] = neutralizedAnswersAllPermutations_loc[serieIndicated-1,participant,:]
        ...
        numberNeutralizedAnswers_loc[participant] = int(sum(neutralizedAnswersIndicatedSeries_loc[participant,:]))
        
    averageScoreQuestions_loc = scoreQuestionsIndicatedSeries_loc.sum(axis=0)/float(numParticipants_loc)    
    return scoreQuestionsIndicatedSeries_loc, averageScoreQuestions_loc, numberCorrectAnswers_loc, numberWrongAnswers_loc, numberBlankAnswers_loc, numberNeutralizedAnswers_loc
